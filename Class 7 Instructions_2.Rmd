...
```{r}
work_student <- spread(Student_data,variable,measure)
```
...
```{r}
student_data_3 <- dplyr::filter(work_student, table == 4)
```
...
```{r}
work_student <- dplyr::mutate(work_student, week = dplyr::ntile(date, 3))
```
...
```{r}
student_week <- work_student %>% dplyr::group_by(week) %>% dplyr::summarise(mean(motivation))
